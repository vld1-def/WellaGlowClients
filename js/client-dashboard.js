/**
 * WELLA GLOW CRM - TOTAL CLIENT ENGINE (V3.0)
 * Свайпи, лояльність, бронювання, відгуки
 */

// 1. КОНФІГУРАЦІЯ ТА СЕСІЯ
const userId = localStorage.getItem('wella_glow_user_id');
if (!userId) window.location.href = 'login.html';

// Глобальні змінні вибору для запису
...
// 2. ГЛОБАЛЬНІ СТИЛІ (Анімації та скидання курсиву)
...
window.updateSidebarUI = function(index) {
    ...
    ids.forEach((id, i) => {
        ...
        if (i === index) {
            ...
            el.classList.remove('nav-active', 'border-l-4', 'border-rose-500');
...
function renderProfileSection(client, history, reviews, upcoming) {
    ...
    container.innerHTML = `
        <div class="max-w-5xl mx-auto space-y-8 animate-fade-in">
            <header><h2 class="text-2xl font-extrabold text-white tracking-tight">Вітаємо, ${firstName}! ✨</h2><p class="text-zinc-500 text-[11px] font-bold uppercase tracking-widest mt-2 leading-none">Твій день для краси сьогодні</p></header>

            <div class="glass-panel p-4 lg:p-8 rounded-[1.5rem] lg:rounded-[2.5rem] border-t-4 border-t-${tier.color} relative overflow-hidden transition-all duration-500">
                ...
                <div class="flex justify-between items-start mb-6 lg:mb-10">
                    <div><p class="text-[9px] text-zinc-500 uppercase font-black tracking-widest leading-none">Статус лояльності</p><h3 class="text-lg lg:text-xl font-black text-${tier.color} mt-2 uppercase tracking-tighter">Glow ${tier.name} Member</h3></div>
                ...
                <div class="flex justify-between items-end">
                    ...
                    <div class="w-10 h-10 lg:w-14 lg:h-14 bg-white/5 rounded-xl lg:rounded-2xl flex items-center justify-center border border-white/5"><i class="fa-solid fa-qrcode text-zinc-400"></i></div>
                </div>
            </div>

            <button onclick="window.scrollToPage(1)" class="neo-gradient w-full py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] text-white shadow-xl transition active:scale-95 italic-none">Записатись зараз</button>
                    ...
                    <div class="p-6 rounded-[2.5rem] bg-rose-500/5 border border-rose-500/20 mb-4 shadow-xl relative overflow-hidden">
                        ...
                        <div class="flex justify-between items-end">
                            <div class="flex gap-6 items-center">
                                <div class="text-center leading-none"><p class="text-2xl font-black text-white tracking-tighter">${new Date(app.appointment_date).getDate()}</p><p class="text-[10px] text-zinc-500 font-bold uppercase mt-1 leading-none">${new Date(app.appointment_date).toLocaleString('uk-UA', {month: 'long'})}</p></div>
...
function renderBookingSection() {
    ...
    container.innerHTML = `
        <div class="max-w-5xl mx-auto space-y-6 animate-fade-in">
            <h2 class="text-2xl font-extrabold text-white tracking-tight uppercase leading-none mb-10">Запис на візит</h2>
            ...
            <div class="glass-panel p-8 rounded-[2.5rem] border-t-4 border-t-rose-500 shadow-2xl">
                ...
                <div class="space-y-4 mb-8">
                    ...
                    <div class="flex justify-between items-center"><span class="text-sm font-black text-white uppercase tracking-tighter leading-none">До сплати</span><span id="sumPrice" class="text-2xl font-black text-emerald-400">₴0</span></div>
                </div>
                <button onclick="window.confirmBooking()" class="neo-gradient w-full py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.3em] text-white">Підтвердити запис</button>
            </div>
        </div>`;
...
window.loadMasterAvailability = async function(el, mid, name) {
    ...
    const today = new Date(); for (let i = 0; i < 14; i++) {
        ...
        const isAv = (shifts?.map(s => s.shift_date) || []).includes(dateStr);
...
window.selectDate = async function(el, date) {
    ...
    const start = parseInt(shift.data.start_time), end = parseInt(shift.data.end_time); const bt = booked.data.map(b => b.appointment_time.substring(0,5)); tGrid.innerHTML = '';
    for (let h = start; h < end; h++) {
        const time = `${h}:00`.padStart(5,'0'); const isB = bt.includes(time);
        tGrid.innerHTML += `<button onclick="${isB ? '' : `window.selectTime(this)`}" class="p-2 rounded-lg border border-white/5 text-[9px] font-black ${isB ? 'opacity-10 bg-zinc-900' : 'bg-white/5 hover:border-rose-500 text-zinc-400'}">${time}</button>`;
    }
...
window.submitReview = async (btn, aid, mid) => {
    ...
    if (!error) { parent.querySelector('.review-input-block').innerHTML = `<div class="p-2 text-center text-emerald-500 font-black uppercase text-[8px]">Дякуємо за відгук! ✨</div>`; setTimeout(() => window.location.reload(), 2500); }
};
